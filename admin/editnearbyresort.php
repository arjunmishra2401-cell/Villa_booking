...
$query = "SELECT * FROM `nearby_resort` WHERE `place_id` = ?";
...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    $update_query = "UPDATE `nearby_resort` SET `resort_id` = ?, `name` = ?, `description` = ?, `location` = ?, `image_url` = ? WHERE `place_id` = ?";
    if ($stmt = mysqli_prepare($conn, $update_query)) {
        mysqli_stmt_bind_param($stmt, "issssi", $hotel_id, $name, $description, $location, $image_url, $place_id);
        if (mysqli_stmt_execute($stmt)) {
            header("Location: dashboard.php?page=nearbyresort.php");
            exit();
        } else {
...
<body>
    ...
    <form method="POST">
        <label for="hotel_id">Resort ID</label>
        <input type="number" id="hotel_id" name="hotel_id" value="<?php echo htmlspecialchars($place['resort_id']); ?>" required>
        ...
        <a href="dashboard.php?page=nearbyresort.php" class="back-btn">Cancel</a>
    </form>
</body>
</html>
