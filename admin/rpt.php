...
$dbname = "villa_booking"; // Update with your actual database name
...
$sql1 = "SELECT hname, DATE(checkin) AS booking_day, COUNT(*) AS total_bookings 
         FROM hotel_booking
         GROUP BY hname, booking_day 
         ORDER BY booking_day ASC";
$result1 = $conn->query($sql1);

// Query 2: Month-wise Booking Ratio
$sql2 = "SELECT hname, DATE_FORMAT(checkin, '%Y-%m') AS booking_month, COUNT(*) AS total_bookings 
         FROM hotel_booking
         GROUP BY hname, booking_month 
...
$sql3 = "SELECT booking_month, (total_bookings / total_overall) * 100 AS booking_ratio FROM (
            SELECT DATE_FORMAT(checkin, '%Y-%m') AS booking_month, COUNT(*) AS total_bookings, 
                   (SELECT COUNT(*) FROM hotel_booking) AS total_overall
            FROM hotel_booking 
...
<h2>Day-wise Booking Report</h2>
<canvas id="dayWiseChart"></canvas>

<h2>Month-wise Booking Report</h2>
<canvas id="monthWiseChart"></canvas>

<h2>Booking Ratio Analysis</h2>
<canvas id="bookingRatioChart"></canvas>

<h2>Highest Booking Ratio</h2>
<canvas id="highestBookingChart"></canvas>

<h2>Lowest Booking Ratio</h2>
