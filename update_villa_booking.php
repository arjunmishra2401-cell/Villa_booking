...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if (empty($booking_id) || empty($new_checkin) || empty($new_checkout)) {
     ...
     // Fetch user email
     $query = "SELECT username, useremail FROM villa_booking WHERE booking_id = ?";
    ...
    // Update the database
    $query = "UPDATE villa_booking SET checkin = ?, checkout = ? WHERE booking_id = ?";
    $stmt = $conn->prepare($query);
    ...
    if ($stmt->execute()) {
        ...
        try {
            ...
            $mail->Subject = "Villa Booking Rescheduled Successfully";
