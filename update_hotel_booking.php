...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if (empty($booking_id) || empty($new_checkin) || empty($new_checkout)) {
        echo "<script> alert('Missing Data!'); setTi</script>";
        exit;
    }

     // Fetch user email
     $query = "SELECT username, useremail FROM hotel_booking WHERE booking_id = ?";
    ...
    // Update the database
    $query = "UPDATE hotel_booking SET checkin = ?, checkout = ? WHERE booking_id = ?";
    $stmt = $conn->prepare($query);
    ...
    if ($stmt->execute()) {
        ...
        try {
            ...
            $mail->Subject = "Hotel Booking Rescheduled Successfully";
