...
if ($qry) {
    ...
    $rname = mysqli_real_escape_string($conn, $resort['name']);

    // Fetch bookings for this resort
    $sql = "SELECT `booking_id`, `room_id`, `rname`, `roomname`, `rprice`, `checkin`, `checkout`, `adult`, `child`, `username`, `fullname`, `userphno`, `useremail`, `payment_id`, `order_id`, `status` 
            FROM `resort_booking` 
            WHERE rname = '$rname'";

    $result = mysqli_query($conn, $sql);
} else {
    die("Error fetching resort details: " . mysqli_error($conn));
}
?>
<!DOCTYPE html>
<html lang="en">
