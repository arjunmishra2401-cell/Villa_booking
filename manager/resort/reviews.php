...
if (!isset($_SESSION['manager_id'])) {
    header("Location: login.php"); // This correctly redirects to /manager/login.php
    exit();
}
...
if (isset($_GET['delete'])) {
    ...
    $stmt = $conn->prepare("DELETE FROM resort_reviews WHERE id = ? AND property_id = ?");
    $stmt->bind_param("ii", $id, $property_id);
    $stmt->execute();
    $stmt->close();
    header("Location: dashboard_resort.php?page=reviews.php");
    exit();
}
...
$result = mysqli_query($conn, "
    SELECT rr.id, u.username, r.name AS resort_name, rr.rating, rr.review
     ...
     JOIN resorts1 r ON rr.property_id = r.id where r.id = $property_id
");
...
<body>
    <h2 class="head">Resort Reviews</h2>
    <table border="1">
        <tr>
            <th>ID</th>
            <th>Username</th>
