...
$query = "SELECT property_id from hotel_managers WHERE id = $id";
$result1 = mysqli_query($conn, $query);
$row = mysqli_fetch_assoc($result1);
$property_id = $row['property_id'];
// Fetch all hotels
$result = mysqli_query($conn, "SELECT place_id, hotel_id, name, description, location, image_url FROM nearby_hotel where hotel_id = $property_id");
?>
...
<head>
    ...
    <style>
        ...
        .add-btn { background-color: #007bff; padding: 10px; margin-top: 20px; display: inline-block; margin-left: 270px;margin-bottom: 10px; }
    </style>
</head>
<body>
    <h2 class="head">Manage Hotels Near By Places</h2>
