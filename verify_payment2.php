...
if ($paymentId && $orderId && $signature) {
    ...
    if (hash_equals($generatedSignature, $signature)) {
        ...
        $updateQuery = "UPDATE villa_booking SET status='paid', payment_id=? WHERE order_id=?";
        $stmt = $conn->prepare($updateQuery);
        $stmt->bind_param("ss", $paymentId, $orderId);
        
        if ($stmt->execute()) {
            
            $query = "SELECT username, useremail, vname, userphno, checkin, checkout, vprice FROM villa_booking WHERE order_id=?";
            ...
            if ($booking) {
                ...
                $hotelName = $booking['vname'];
                $userPhone = $booking['userphno'];
                $checkIn = $booking['checkin'];
                $checkOut = $booking['checkout'];
                $price = $booking['vprice'];
                ...
                if (sendPaymentConfirmationEmail($email, $username, $hotelName, $paymentId, $orderId, $invoicePath)) {
                    ...
                    echo "<script>alert('Payment successful, but email could not be sent.'); 
                        setTimeout(function() { window.location.href = 'villas.php'; }, 2000);</script>";
                }
            }
        } else {
        ...
        echo "<script>
            ...
            setTimeout(function() { window.location.href = 'villas.php'; }, 2000);
        </script>";
    }
} else {
    echo "<script>
