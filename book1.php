...
if (isset($_GET['room_id'])) {
    ...
    // Fetch Room Details
    $queryRoom = "SELECT * FROM rooms2 WHERE room_id = ?";
    ...
    if ($_SERVER['REQUEST_METHOD'] === 'POST') {
        ...
        if ($availability['booking_count'] >= 5) {
             echo "<script>alert('This room is fully booked for the selected dates.'); window.location.href = 'book1.php?room_id=$roomId';</script>";
             exit();
        }
        ...
        if (isset($responseArray['id'])) {
            ...
            // Insert 'Pending' Booking with PENDING status to fix DB error
            $insertQuery = "INSERT INTO resort_booking (username, fullname, room_id, roomname, rname, rprice, checkin, checkout, adult, child, userphno, useremail, order_id, payment_id, status) 
                            VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)";
            
            $stmtInsert = $conn->prepare($insertQuery);
            
            $stmtInsert->bind_param("ssisssssiiissss", 
                $username, $name, $roomId, $room['room_type'], $room['resort_name'], 
    ...
    // If no booking_id and no room_id, invalid request
    header("Location: index.php");
    exit();
}
...
<body>
    ...
    <form method="POST" class="payform">
        ...
        <input type="text" value="<?= htmlspecialchars($room['resort_name']); ?>" readonly>

        <label>Room Name:</label>
        <input type="text" value="<?= htmlspecialchars($room['room_type']); ?>" readonly>

        <label>Price Per Night:</label>
        <input type="text" value="₹<?= number_format($room['price_per_night'], 2); ?>" readonly>
